resource "aws_cloudwatch_metric_alarm" "logs-1-500-error-1-minute-warning" {
  ...
  metric_name         = aws_cloudwatch_log_metric_filter.500-errors.metric_transformation[0].name
  namespace           = aws_cloudwatch_log_metric_filter.500-errors.metric_transformation[0].namespace
  period              = "60"
  statistic           = "Sum"
  threshold           = 1
...
resource "aws_cloudwatch_metric_alarm" "logs-1-429-error-1-minute-warning" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = aws_cloudwatch_log_metric_filter.429-errors.metric_transformation[0].name
  namespace           = aws_cloudwatch_log_metric_filter.429-errors.metric_transformation[0].namespace
  period              = "60"
  statistic           = "Sum"
  threshold           = 1
  treat_missing_data  = "notBreaching"
