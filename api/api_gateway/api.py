...
def get_db_version(context_session):
...
@app.get("/healthcheck")
async def healthcheck(context_session: Session = Depends(get_db)):
    try:
        full_name = get_db_version(context_session)
...
@app.get("/lists")
async def lists(context_session: Session = Depends(get_db)):
    with context_session as session:
...
async def lists_by_service(service_id, context_session: Session = Depends(get_db)):
    with context_session as session:
...
async def create_list(
    ...
    response: Response,
    context_session: Session = Depends(get_db),
):
...
async def delete_list(
    list_id, response: Response, context_session: Session = Depends(get_db)
):
    with context_session as session:
        try:
...
async def confirm(
    subscription_id, response: Response, context_session: Session = Depends(get_db)
):
    with context_session as session:
        try:
...
async def unsubscribe(
    unsubscription_id, response: Response, context_session: Session = Depends(get_db)
):
    notifications_client = get_notify_client()
    with context_session as session:
        try:
